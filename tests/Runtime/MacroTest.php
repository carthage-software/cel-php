...
class MacroTest extends RuntimeTestCase
{
    ...
    public static function provideEvaluationCases(): iterable
    {
        ...
        yield 'Macro has: map key exists with null value' => [
            'has(params.optional)',
            ['params' => Value::from(['optional' => null])],
            new BooleanValue(true),
        ];
        ...
        yield 'Macro all error: target is not list or map' => [
            '1.all(x, x > 0)',
            [],
            new InvalidMacroCallException('The `all` macro requires a list or map target, got `int`', new Span(0, 1)),
        ];
        ...
        yield 'Macro exists (map): one key matches' => [
            '{"a":1, "bb":2}.exists(k, k.size() > 1)',
            [],
            new BooleanValue(true),
        ];
        yield 'Macro exists (map): no key matches' => [
            '{"a":1, "b":2}.exists(k, k.size() > 1)',
            [],
            new BooleanValue(false),
        ];
