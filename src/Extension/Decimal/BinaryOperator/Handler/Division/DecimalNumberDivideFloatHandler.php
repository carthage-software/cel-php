...
/**
 * Handles division of a DecimalNumber by a Float.
 */
final readonly class DecimalNumberDivideFloatHandler implements BinaryOperatorOverloadHandlerInterface
{
    /**
     ...
     *
     * @return Value The result of dividing the DecimalNumber by the Float.
     *
    ...
    public function __invoke(BinaryExpression $expression, Value $left, Value $right): Value
    {
        ...
        try {
            $result = $left->message->getInner()->div(DecimalFactory::from((string) $right->value));
        } catch (Throwable $e) {
