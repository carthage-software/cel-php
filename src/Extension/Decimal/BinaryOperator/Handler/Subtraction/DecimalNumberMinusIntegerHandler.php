...
/**
 * Handles subtraction of an Integer from a DecimalNumber.
 */
final readonly class DecimalNumberMinusIntegerHandler implements BinaryOperatorOverloadHandlerInterface
{
    /**
     ...
     *
     * @return Value The result of subtracting the Integer from the DecimalNumber.
     *
    ...
    public function __invoke(BinaryExpression $expression, Value $left, Value $right): Value
    {
        ...
        try {
            $result = $left->message->getInner()->sub(new Decimal((string) $right->value));
        } catch (Throwable $e) {
